...
BBCMicrobit.is = function(peripheral) {
  ...
  return (
    localName !== undefined &&
    localName.indexOf("BBC micro:bit") !== -1 &&
    localName.indexOf(root.基板名) !== -1
  );
};
...
BBCMicrobit.prototype.connectAndSetUp = function(callback) {
  BBCMicrobit.super_.prototype.connectAndSetUp.call(
    this,
    function(error) {
      if (error) {
      ...
      if (this.hasIoPinService()) {
        this.readPinAdConfiguration(
          function(error) {
            if (error) {
              return callback(error);
            }

            this.readPinIoConfiguration(
              function(error) {
                if (error) {
                  return callback(error);
                }
