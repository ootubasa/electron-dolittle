(function() {
  ...
  root.ファイル.書く = function(val) {
    fs.appendFileSync(__dirname + "/workspace/" + root.ファイル.名前, val);
  };
  root.ファイル.改行 = function() {
    fs.appendFileSync(__dirname + "/workspace/" + root.ファイル.名前, "\n");
  };
  root.ファイル.読む = function() {
    fs.readFileSync(__dirname + "/workspace/" + root.ファイル.名前);
  };
}
  .checkerror()
  .apply(root, []));
